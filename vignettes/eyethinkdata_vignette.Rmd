...
**data$data_task** contains all of the data from you gorilla tasks, the blue blobs in your experiment. This is the raw data from gorilla all collated together, with a few other columns added. For example, **pid** is the participant identity (anonymised). **rt** is the reaction time column. 
...
iat_data <-  data$data_task[metadata %in% c("congruent","incongruent") & Attempt==1 & display=="trials",  # row selection
                                   .(pid, trial=Trial.Number, congruency=metadata,accuracy=Correct,rt,                # columns    
...
Now we can plot the data with the generic ploting function, mypirate. You name the columns/conditions that you want to plot on the x axis, and as a colour contrast.
...
Since we have a dot per trial, this is a bit crowded. You can first average over people, and also turn elements on and off in mypirate. and set options to flip the coordinates

```{r}
mypirate(data=iat_data[accuracy==1 & rt<2000],
         colour_condition = "congruency",x_condition = "firstblock",dv = "rt",pid_average = T,
         violin = F,bar=T,cflip = T)  # no dots but draw some bars
```
...
anv <- afex::aov_ez(data=iat_data[accuracy==1 & rt<2000],fun_aggregate = mean,
 ...
 plot_model(anv,outp = NULL,bars=F) # you can pass mypirate parameters to this function as above
```

For the IAT there is a function that will do all the processing and analyis we've done above in one 
